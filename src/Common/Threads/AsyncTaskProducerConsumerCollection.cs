...
namespace Zhai.Famil.Common.Threads
{
    ...
    /// 异步任务队列
    /// </summary>
    public class AsyncTaskProducerConsumerCollection<TProducerConsumerCollection> : IDisposable
        where TProducerConsumerCollection : IProducerConsumerCollection<AwaitableTask>, new()
    {
        private bool _isDisposed;
        private readonly TProducerConsumerCollection _items;
        ...
        /// 异步任务队列
        /// </summary>
        public AsyncTaskProducerConsumerCollection()
        {
            _items = new TProducerConsumerCollection();
        ...
        private bool TryGetNextTask(out AwaitableTask task)
        {
            ...
            {
                if (_items.TryTake(out task) && (!AutoCancelPreviousTask || _items.Count == 0)) return true;
                task.Cancel();
            }
            return false;
        }
        ...
        /// <param name="function"></param>
        /// <returns></returns>
        public void Add<TResult>(Func<TResult> function) 
            => Run(function);
        ...
        /// 析构任务队列
        /// </summary>
        ~AsyncTaskProducerConsumerCollection() => Dispose(false);
    ...
    /// 异步任务队列(先进后出)
    /// </summary>
    public class AsyncTaskQueue : AsyncTaskProducerConsumerCollection<ConcurrentQueue<AwaitableTask>>
    { 
    ...
    /// 异步任务队列(先进先出)
    /// </summary>
    public class AsyncTaskStack : AsyncTaskProducerConsumerCollection<ConcurrentStack<AwaitableTask>>
    { }
}
